...
class CashFlowForecaster:
    ...
    def forecast_daily_balances(self,
        ...
        salary_events = [e for e in self.events if e.event_type == 'income' and 'salary' in (e.category + ' ' + e.description).lower() and e.status == 'settled']
        ...
        for cat, evs in cat_debit_events.items():
            ...
            is_recurring = len(evs) > 1 or any(e.flexibility in ['stoppable', 'reducible', 'reducible_or_stoppable'] for e in evs) or cat in self.profile.expense_categories_to_protect
